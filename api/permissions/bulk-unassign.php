...
try {
    ...
    // Build placeholders for bulk delete
    $placeholders = [];
    $params = ['role_id' => $roleId];
    foreach ($permissionIds as $index => $pid) {
        $placeholders[] = ":permission_id_{$index}";
        $params["permission_id_{$index}"] = $pid;
    }
    
    $sql = "DELETE FROM role_permissions WHERE role_id = :role_id AND permission_id IN (" . implode(', ', $placeholders) . ")";
